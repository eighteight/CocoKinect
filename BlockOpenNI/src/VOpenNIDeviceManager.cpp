...
***/
    ...
    void OpenNIDeviceManager::setFrame(size_t frame, uint devNum){
        if (!player) return;
        player.SeekToFrame(mDepthGenList[devNum].GetName(), XnUInt32(frame), XN_PLAYER_SEEK_SET);
        player.SeekToFrame(mImageGenList[devNum].GetName(), XnUInt32(frame), XN_PLAYER_SEEK_SET);
        
    }
    
    size_t OpenNIDeviceManager::getTotalNumFrames(uint devNum){
        XnUInt32 totalFrames = 0;
        if(player){
            player.GetNumFrames(mDepthGenList[devNum].GetName(), totalFrames);
        }
        return (size_t)totalFrames;
    }

    bool OpenNIDeviceManager::createDevice(const XnChar* strFileName, int nodeTypeFlags, int deviceNum)
    {
        int nRetVal = _context.Init();
        CHECK_RC( nRetVal, "Init" );
        
        nRetVal = _context.OpenFileRecording( strFileName );
        ...
        // Player nodeの取得
        nRetVal = _context.CreateAnyProductionTree(XN_NODE_TYPE_PLAYER,&mQuery,  player);
        //_context.OpenFileRecording(strFileName, player);
        ...
        for( xn::NodeInfoList::Iterator it = nodeList.Begin();
            ...
            if( (*it).GetDescription().Type == XN_NODE_TYPE_IMAGE ){
                ...
                nRetVal = _context.FindExistingNode(XN_NODE_TYPE_IMAGE, gen);
                ...
                nRetVal = _context.FindExistingNode(XN_NODE_TYPE_DEPTH, gen);
        ...
        {
            V::OpenNIDeviceRef dev = V::OpenNIDeviceRef( new V::OpenNIDevice( deviceNum, this) );
            mDevices.push_back( dev );
        }
        
        if( nodeTypeFlags & V::NODE_TYPE_DEPTH )
        {
            xn::DepthGenerator gen;
            status = _context.CreateAnyProductionTree( NODE_TYPE_DEPTH, &mQuery, gen );
        ...
        if( nodeTypeFlags & V::NODE_TYPE_IMAGE )
        {
            xn::ImageGenerator gen;
            status = _context.CreateAnyProductionTree( NODE_TYPE_IMAGE, &mQuery, gen );
        ...
        if( nodeTypeFlags & V::NODE_TYPE_USER )
        {
        ...
        if( nodeTypeFlags & V::NODE_TYPE_SCENE )
        {
        ...
        // Initialize devices
        int ff = 0;
        for( V::OpenNIDeviceList::iterator it=mDevices.begin(); it!=mDevices.end(); ++it ) {
            if (ff==0 && deviceNum ==1){
                ff++;
                continue;
            }
            bool result = (*it)->init( nodeTypeFlags, 4, 4 );
            if( !result ){
	...
	bool OpenNIDeviceManager::createDevices( uint32_t deviceCount, int nodeTypeFlags, int colorResolution, int depthResolution )
	{
		...
		XnStatus status = _context.EnumerateProductionTrees( XN_NODE_TYPE_DEVICE, NULL, device_nodes, NULL );
        CHECK_RC( status, "Production Tree" );
		for( xn::NodeInfoList::Iterator nodeIt=device_nodes.Begin(); nodeIt!=device_nodes.End(); ++nodeIt ) 
		{
            ...
            CHECK_RC( status, "Add needed node" );
			std::stringstream ss;
...
//			mDeviceList.push_back( sdev );
	...
	void OpenNIDeviceManager::Release( void )
	{
        ...
        if (player)
            player.Release();
		// stop generators
		//XnStatus status = _context.StopGeneratingAll();
        //CHECK_RC(status, "Stopping All");
        		//_context.Shutdown();
		...
		for ( size_t i = 0; i < mImageGenList.size(); i++ ){
            mImageGenList[i].StopGenerating();
...
//		mSceneMDList.clear();
  ...
  //        boost::this_thread::sleep( boost::posix_time::millisec(1) ); 
    ...
    void OpenNIDeviceManager::updatePositions(boost::unordered_map<int, std::vector<cinder::Vec3f> > &userPositions){

        for( OpenNIUserList::iterator usr = mUserList.begin(); usr != mUserList.end(); ++usr ){
            if ((*usr)->isTracking()){
                (*usr)->updatePositions( userPositions );
            }
        }
    }
